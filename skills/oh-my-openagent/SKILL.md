...
description: Configure and improve oh-my-openagent for the current user. Use when users want to tune agents, models, prompts, custom agents, skills, MCPs, presets, or plugin behavior. Also use when recurring workflow friction suggests a safe config or prompt improvement.
---

# oh-my-openagent Configuration Skill
...
- `~/.config/opencode/oh-my-openagent.json` or `.jsonc`
...
oh-my-openagent is configured through a plugin config file, usually:
...
| `~/.config/opencode/oh-my-openagent.jsonc` | User plugin config with comments/trailing commas; takes precedence over `.json` |
| `~/.config/opencode/oh-my-openagent.json` | User plugin config generated by installer |
| `<project>/.opencode/oh-my-openagent.json` | Project-local plugin overrides checked before user config |
| `~/.config/opencode/oh-my-openagent/{agent}.md` | Full prompt replacement for a built-in agent |
| `~/.config/opencode/oh-my-openagent/{agent}_append.md` | Append-only prompt tuning for a built-in agent |
| `~/.config/opencode/oh-my-openagent/{preset}/{agent}.md` | Preset-specific full prompt replacement |
| `~/.config/opencode/oh-my-openagent/{preset}/{agent}_append.md` | Preset-specific append-only prompt tuning |
...
1. If a `preset` is active, check
   `~/.config/opencode/oh-my-openagent/{preset}/` first.
2. Fall back to `~/.config/opencode/oh-my-openagent/`.
...
- Built-in agent `prompt` and `orchestratorPrompt` fields are **not** supported
  in `oh-my-openagent.json[c]`; use markdown prompt override files instead.
...
```text
~/.config/opencode/oh-my-openagent/orchestrator_append.md
```

or for only the `openai` preset:

```text
~/.config/opencode/oh-my-openagent/openai/orchestrator_append.md
```
...
```text
~/.config/opencode/oh-my-openagent/orchestrator.md
```
...
1. **Inspect current setup**
   - Read the existing oh-my-openagent config file.
...
~/.config/opencode/oh-my-openagent/orchestrator_append.md. Apply it?
```
...
public API changes to it. Apply to your oh-my-openagent config?
```
